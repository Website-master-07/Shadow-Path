...
function persist() {
  localStorage.setItem('sp_state', JSON.stringify({
    mode: S.mode, student: S.student, requests: S.requests, proProfile: S.proProfile,
  }));
}
...
function proCard(p) {
  const req = S.requests.find(r => r.proId === p.id);
  ...
  return `
    <div class="pro-card" onclick="openProfileModal(${p.id})">
      <div class="pro-card-top" style="background:linear-gradient(${p.gradient})"></div>
      <div class="pro-card-body">
        <div class="pro-avatar-row">
          <div class="pro-avatar" style="background:linear-gradient(${p.gradient})">${p.initials}</div>
          <div class="pro-avail-dot ${availClass}">${availText}</div>
        </div>
        <div class="pro-name">${p.name}</div>
        <div class="pro-role">${p.title} · ${p.company}</div>
        <div class="pro-bio">${p.bio}</div>
        <div class="pro-tags">
          ...
          <span class="badge badge-cyan">📍 ${p.location}</span>
        </div>
        <div class="pro-card-btns">
          <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openProfileModal(${p.id})">View Profile</button>
          ${req
            ? `<button class="btn btn-sm" style="background:var(--green-light);color:#065f46;border-color:var(--green-light)" disabled>Sent ✓</button>`
            : `<button class="btn btn-indigo btn-sm" onclick="event.stopPropagation();openReqModal(${p.id})">Request Shadow</button>`
          }
        </div>
      </div>
    </div>
...
function openProfileModal(proId) {
  ...
  document.getElementById('profile-modal-box').innerHTML = `
    ...
    <div class="pm-body">
      ...
      <div class="pm-role">${p.title} · ${p.company}</div>
      <div class="pm-tags">
        <span class="badge badge-indigo">${shadowLabel(p.shadowType)}</span>
        <span class="badge badge-gray">${fieldEmoji(p.field)} ${fieldName(p.field)}</span>
        <span class="badge badge-cyan">📍 ${p.location}</span>
        <span class="badge badge-gray">💼 ${p.experience}</span>
      </div>
      <p class="pm-bio">${p.bio}</p>
      ...
      <div class="pm-actions">
        ...
        ${req
          ...
          : `<button class="btn btn-indigo" onclick="closeProfileModal(null);openReqModal(${p.id})">Request Shadow Opportunity</button>`
        }
      </div>
    </div>
  `;
...
function openReqModal(proId) {
  ...
  document.getElementById('req-modal-body').innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:.85rem;padding:.75rem;background:var(--bg);border-radius:var(--r-sm)">
      ...
      <div><div style="font-weight:700;font-size:.9rem">${p.name}</div><div style="font-size:.78rem;color:var(--text-muted)">${p.title} · ${p.company}</div></div>
    </div>
    <div class="form-grp">
      <label class="form-lbl">Your full name</label>
      <input type="text" id="rq-name" class="inp" placeholder="Your full name" value="${S.student?.name || ''}">
    </div>
    <div class="form-grp">
    ...
    <div class="form-grp">
      ...
      <textarea id="rq-note" class="inp inp-ta" placeholder="Any special requests or notes..." style="min-height:80px"></textarea>
    </div>
...
function submitRequest() {
  const proId = +document.getElementById('req-pro-id').value;
  ...
  const req = {
    id: Date.now(),
    proId, proName: p.name, proTitle: p.title, proCompany: p.company,
    proInitials: p.initials, proGradient: p.gradient,
    studentName: name || S.student?.name || 'Student',
    studentEmail: email,
    type, date, note,
    status: 'pending',
    ...
    messages: [
      {
        from: 'student',
        text: `Hi ${p.name.split(' ')[0]}! My name is ${name || S.student?.name || 'a student'} and I'd love to ${type === 'Shadow' ? 'shadow you' : type === 'Mentorship' ? 'connect for mentorship' : 'ask you a few questions'}. ${note}`,
...
function renderMessaging() {
  ...
  list.innerHTML = S.requests.map(r => {
    ...
    const active = S.activeConv === r.id;
    return `
      <div class="conv-item ${active ? 'active' : ''}" onclick="selectConv(${r.id})">
        <div class="conv-meta">
          <div class="conv-name">${r.proName}</div>
          <div class="conv-time">${fmtDate(r.createdAt)}</div>
        </div>
        <div style="display:flex;align-items:center;gap:.4rem;margin-bottom:.15rem">
          ${statusBadge(r.status)}
        </div>
        <div class="conv-last">${last ? (last.from === 'student' ? 'You: ' : '') + last.text : ''}</div>
      </div>
    `;
  }).join('');

  if (S.activeConv) {
    const r = S.requests.find(x => x.id === S.activeConv);
...
function selectConv(id) {
  ...
  document.querySelectorAll('.conv-item').forEach((el, i) => {
    el.classList.toggle('active', S.requests[i]?.id === id);
  });
  renderConvDetail(r);
}
...
function renderConvDetail(r) {
  ...
  document.getElementById('msg-main').innerHTML = `
    <div class="msg-detail-header">
      ...
      <div style="flex:1">
        <div class="msg-d-name">${r.proName}</div>
        <div class="msg-d-role">${r.proTitle} · ${r.proCompany}</div>
      </div>
    ...
    <div class="msg-thread" id="msg-thread-${r.id}">
      ${r.messages.map(m => msgBubble(m)).join('')}
    </div>
    <div class="msg-compose">
      <textarea class="msg-compose-inp" id="compose-${r.id}" placeholder="Type a message..." rows="1" onkeydown="onComposeKey(event,${r.id})"></textarea>
      <button class="btn btn-indigo btn-sm" onclick="sendMsg(${r.id})">Send</button>
    </div>
  `;
...
function sendMsg(id) {
  ...
  if (!req) return;
  req.messages.push({ from: 'student', text, time: new Date().toISOString() });
  persist();
  const thread = document.getElementById('msg-thread-' + id);
  if (thread) {
    const div = document.createElement('div');
    div.innerHTML = msgBubble({ from:'student', text, time: new Date().toISOString() });
  ...
  document.getElementById('d-recommended').innerHTML = showPros.map(p => `
    <div class="mini-pro">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div class="mini-role">${p.title}</div>
      </div>
    ...
    reqEl.innerHTML = S.requests.slice(0, 5).map(r => `
      <div class="req-mini" onclick="goTo('messaging')">
        <div class="req-mini-head">
          <div><div class="req-mini-name">${r.proName}</div><div class="req-mini-role">${r.proTitle}</div></div>
          ${statusBadge(r.status)}
        </div>
      </div>
    `).join('');
    ...
    <div class="inbox-card">
      <div class="inbox-card-head">
        <div class="inbox-student-info">
          ...
          <div>
            <div class="inbox-s-name">${r.studentName}</div>
            <div class="inbox-s-meta">${r.type} Request · ${fmtDateShort(r.date)} ${r.date ? '· ' + fmtDate(r.createdAt) : ''}</div>
          </div>
        </div>
        ${statusBadge(r.status)}
      </div>
      ...
      <div class="inbox-actions">
        ${r.status === 'pending' ? `
          <button class="btn btn-success btn-sm" onclick="inboxAction(${r.id},'accepted')">Accept</button>
          <button class="btn btn-danger btn-sm" onclick="inboxAction(${r.id},'declined')">Decline</button>
          <button class="btn btn-ghost btn-sm" onclick="selectConv(${r.id});goTo('messaging')">Message Student</button>
        ` : `
          <span class="text-muted" style="font-size:.8rem">${r.status === 'accepted' ? '✅ Accepted' : '❌ Declined'}</span>
          <button class="btn btn-ghost btn-sm" onclick="selectConv(${r.id});goTo('messaging')">View Messages</button>
        `}
      </div>
    </div>
  `).join('');
