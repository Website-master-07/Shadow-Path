...
let _prosLoaded = false;    // true once the first pros snapshot arrives

function skeletonProCard() {
  return `<div class="pro-card skeleton-card" aria-hidden="true">
    <div class="pro-card-top sk-block" style="height:72px"></div>
    <div class="pro-card-body">
      <div class="pro-avatar-row" style="margin-top:-32px;margin-bottom:.75rem">
        <div class="sk-circle" style="width:60px;height:60px;border:3px solid white"></div>
        <div class="sk-block" style="width:90px;height:24px;border-radius:99px"></div>
      </div>
      <div class="sk-block" style="width:65%;height:15px;margin-bottom:8px"></div>
      <div class="sk-block" style="width:45%;height:12px;margin-bottom:14px"></div>
      <div class="sk-block" style="height:11px;margin-bottom:5px"></div>
      <div class="sk-block" style="width:80%;height:11px;margin-bottom:18px"></div>
      <div style="display:flex;gap:6px;margin-bottom:18px">
        <div class="sk-block" style="width:72px;height:22px;border-radius:99px"></div>
        <div class="sk-block" style="width:88px;height:22px;border-radius:99px"></div>
      </div>
      <div style="display:flex;gap:8px">
        <div class="sk-block" style="flex:1;height:34px;border-radius:8px"></div>
        <div class="sk-block" style="flex:1;height:34px;border-radius:8px"></div>
      </div>
    </div>
  </div>`;
...
function findPro(id) {
  ...
  const found = PROFESSIONALS.find(x => x.id === numId || x.id === id)
             || _firestorePros.find(x => x.firestoreId === id || x.id === id);
  if (!found) console.warn('[findPro] Not found — id:', id, '| _firestorePros count:', _firestorePros.length, '| ids:', _firestorePros.map(p => p.id));
  return found;
}
...
function filterProfessionals() {
  ...
  if (!_prosLoaded && _firestorePros.length === 0 && typeof dbListenPros === 'function') {
    // Show skeleton while waiting for Firestore first snapshot
    document.getElementById('pro-count').textContent = 'Loading professionals…';
    document.getElementById('pros-grid').innerHTML = Array(6).fill(skeletonProCard()).join('');
    return;
  }
...
function proCard(p) {
  ...
  const reqIdJs = req ? (typeof req.id === 'number' ? req.id : `'${req.id}'`) : null;
  return `
    <div class="pro-card" onclick="openProfileModal(${idJs})">
      ...
      <div class="pro-card-body">
        ...
        <div class="pro-card-btns">
          <button class="btn btn-indigo btn-sm" onclick="event.stopPropagation();${
            req ? `goToConv(${reqIdJs})` : `messageProDirect(${idJs})`
          }">Message</button>
          ${req
            ? `<button class="btn btn-sm btn-ghost" style="color:var(--green)" disabled>✓ Requested</button>`
            : `<button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openReqModal(${idJs})">Request Shadow</button>`
          }
        </div>
      </div>
    </div>
...
function openProfileModal(proId) {
  ...
  if (!p) { toast('Could not load profile. Please refresh and try again.'); return; }
  const idJs = typeof p.id === 'number' ? p.id : `'${p.id}'`;
  const req = S.requests.find(r => String(r.proId) === String(p.id));
  ...
  document.getElementById('profile-modal-box').innerHTML = `
    ...
    <div class="pm-body">
      ...
      <div class="pm-actions">
        ...
        <button class="btn btn-indigo" onclick="closeProfileModal(null);${
          req ? `goToConv(${typeof req.id === 'number' ? req.id : `'${req.id}'`})` : `messageProDirect(${idJs})`
        }">Message</button>
        ${req
          ? `<button class="btn btn-sm btn-ghost" style="color:var(--green)" disabled>✓ Requested</button>`
          : `<button class="btn btn-ghost" onclick="closeProfileModal(null);openReqModal(${idJs})">Request Shadow</button>`
        }
      </div>
    </div>
  `;
...
function openReqModal(proId) {
  // Auto-create student profile if somehow missing (shouldn't happen after Google login)
  if (!S.student) {
    const u = authGetCurrentUser();
    if (u) {
      S.student = { name: u.name || u.email || 'Student', email: u.email || '', grade: '', location: '', interests: [] };
      persist();
    }
  }
  ...
  const p = findPro(proId);
  if (!p) {
    toast('Could not find this professional. Please refresh the page and try again.');
    return;
  }
...
/** Start a direct message conversation with a pro (auto-creates request if none exists) */
function messageProDirect(proId) {
  const p = findPro(proId);
  if (!p) { toast('Professional not found.'); return; }

  const existing = S.requests.find(r => String(r.proId) === String(p.id));
  if (existing) { goToConv(existing.id); return; }

  const user = authGetCurrentUser();
  if (!user) { toast('Please sign in first.'); return; }
  if (!S.student && S.mode !== 'pro') { toast('Set up your profile first.'); goTo('dashboard'); return; }

  const studentName = S.student?.name || user.name || 'Student';
  const reqData = {
    proId: p.id,
    proUid: p.firestoreId || null,
    studentUid: user.uid || 'demo',
    proName: p.name, proTitle: p.title, proCompany: p.company || '',
    proInitials: p.initials, proGradient: p.gradient,
    studentName, studentEmail: S.student?.email || user.email || '',
    type: 'Message', date: new Date().toISOString(), note: '',
  };

  if (typeof dbSaveRequest === 'function') {
    dbSaveRequest(reqData).then(ref => {
      toast('Conversation started!');
      goToConv(ref.id);
    }).catch(() => toast('Failed to start conversation. Try again.'));
  } else {
    const req = { id: Date.now(), ...reqData, status: 'pending', createdAt: new Date().toISOString(), messages: [] };
    S.requests.unshift(req);
    persist();
    goToConv(req.id);
  }
...
function filterConversations() {
  ...
  if (!q) { renderMessaging(); return; }
  const list = document.getElementById('conv-list');
  const filtered = S.requests.filter(r =>
    esc(r.proName).toLowerCase().includes(q) ||
    esc(r.proTitle).toLowerCase().includes(q) ||
    esc(r.studentName).toLowerCase().includes(q)
  );
    ...
    return `
      <div class="conv-item ${active ? 'active' : ''}" onclick="selectConv(${idJs})">
        <div class="conv-meta">
          ...
          <div class="conv-time">${r.createdAt ? fmtDate(r.createdAt) : ''}</div>
        </div>
        <div style="display:flex;align-items:center;gap:.4rem;margin-bottom:.15rem">${statusBadge(r.status)}</div>
        <div class="conv-last">${last ? (last.from === 'student' ? 'You: ' : '') + esc(last.text) : esc(r.note || '')}</div>
      </div>
    `;
  }).join('') || `<div class="empty-block" style="padding:1.5rem"><p class="text-muted" style="font-size:.82rem;text-align:center">No conversations match.</p></div>`;
}
...
function renderMessaging() {
  ...
  if (S.requests.length === 0) {
    ...
    list.innerHTML = `<div class="empty-block" style="padding:2rem">
      ...
      <p class="text-muted" style="font-size:.82rem;text-align:center">${isPro ? 'No messages yet.<br>Students who request you will appear here.' : 'No conversations yet.<br>Request a shadow to start!'}</p>
    </div>`;
    document.getElementById('msg-main').innerHTML = `<div class="msg-empty">
      <div class="empty-icon" style="width:64px;height:64px"><svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>
      <h3>${isPro ? 'Your inbox is empty' : 'No conversations yet'}</h3>
      <p class="text-muted" style="font-size:.875rem">${isPro ? 'When students send you shadow requests, they\'ll appear here.' : 'Find a professional and send your first shadow request!'}</p>
      ${!isPro ? `<button class="btn btn-indigo btn-sm" onclick="goTo('professionals')">Browse Professionals</button>` : ''}
    </div>`;
    return;
  }
...
function renderConvDetail(r) {
  ...
  document.getElementById('msg-main').innerHTML = `
    ...
    <div class="msg-compose">
      <textarea class="msg-compose-inp" id="compose-${r.id}" placeholder="Type a message…" rows="1" maxlength="1000" onkeydown="onComposeKey(event,${idJs})" oninput="this.style.height='auto';this.style.height=Math.min(this.scrollHeight,120)+'px'"></textarea>
      <button class="btn btn-indigo btn-sm" onclick="sendMsg(${idJs})">
...
function msgBubble(m) {
  const mine = S.mode === 'pro' ? m.from === 'pro' : m.from === 'student';
  const senderLabel = !mine && m.senderName ? `<div class="bubble-sender">${esc(m.senderName)}</div>` : '';
...
function sendMsg(id) {
  ...
  if (typeof dbSendMessage === 'function') {
    // Optimistic UI: show bubble immediately without waiting for listener
    const thread = document.getElementById('msg-thread-' + id);
    if (thread) {
      const el = document.createElement('div');
      el.innerHTML = msgBubble({ from, senderName, text, time: new Date() });
      thread.appendChild(el.firstElementChild);
      thread.scrollTop = thread.scrollHeight;
    }
...
function renderDashboard() {
  ...
  const allPros = [...PROFESSIONALS, ..._firestorePros];
  const matched = allPros.filter(p => interests.includes(p.field)).slice(0, 4);
  const showPros = matched.length ? matched : allPros.slice(0, 4);
  document.getElementById('d-recommended').innerHTML = showPros.map(p => {
    const idJs = typeof p.id === 'number' ? p.id : `'${p.id}'`;
    return `
      <div class="mini-pro">
        <div class="mini-avatar" style="background:linear-gradient(${p.gradient})">${esc(p.initials)}</div>
        <div style="flex:1;min-width:0">
          <div class="mini-name">${esc(p.name)}</div>
          <div class="mini-role">${esc(p.title)}</div>
        </div>
        <button class="btn btn-sm btn-indigo" onclick="openReqModal(${idJs})">Request</button>
      </div>
    `;
  }).join('');
...
function setInboxTab(tab, el) {
  ...
  if (filtered.length === 0) {
    const emptyMsg = S.mode === 'pro'
      ? `When students request to shadow you, they'll appear here.`
      : `You haven't sent any ${tab === 'all' ? '' : tab + ' '}requests yet.`;
  ...
  list.innerHTML = filtered.map(r => {
    const idJs = typeof r.id === 'number' ? r.id : `'${r.id}'`;
    const createdStr = r.createdAt ? fmtDate(r.createdAt) : '';
    return `
...
function _toDate(v) {
  ...
  if (v?.seconds) return new Date(v.seconds * 1000); // Firestore Timestamp plain object
  return new Date(v);
}
function fmtDate(v)      { const d = _toDate(v); return d ? d.toLocaleDateString('en-US',{month:'short',day:'numeric'}) : ''; }
function fmtDateShort(v) { if (!v) return ''; try { const d = _toDate(v); return d.toLocaleDateString('en-US',{month:'short',day:'numeric'}); } catch(e) { return String(v); } }
function fmtTime(v)      { const d = _toDate(v); return d ? d.toLocaleString('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}) : ''; }

// Update nav badge counts based on pending requests
function updateNavBadges() {
  const pending = S.requests.filter(r => r.status === 'pending').length;
  const isPro = S.mode === 'pro';
  const badges = [
    { id: 'nav-msg-badge',   show: !isPro && pending > 0, val: pending },
    { id: 'nav-inbox-badge', show:  isPro && pending > 0, val: pending },
    { id: 'nav-dash-badge',  show: !isPro && pending > 0, val: pending },
    { id: 'mob-msg-badge',   show: !isPro && pending > 0, val: pending },
    { id: 'mob-inbox-badge', show:  isPro && pending > 0, val: pending },
    { id: 'mob-dash-badge',  show: !isPro && pending > 0, val: pending },
  ];
  badges.forEach(({ id, show, val }) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.textContent = val;
...
function initTheme() {
  const saved = localStorage.getItem('sp_theme') || 'system';
  applyTheme(saved);
  // Respond to OS-level theme changes when set to system
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
    if ((localStorage.getItem('sp_theme') || 'system') === 'system') applyTheme('system');
  });
}
...
function openSettings() {
  ...
  // Sync active theme option
  const saved = localStorage.getItem('sp_theme') || 'system';
  ['light','dark','system'].forEach(t => {
    document.getElementById('theme-opt-' + t)?.classList.toggle('active', t === saved);
  });
}

function closeSettings(e) {
  if (e === null || e.target === document.getElementById('settings-overlay')) {
