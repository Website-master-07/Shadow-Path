...
// Test mode: bypass Firebase even when FIREBASE_READY is true
let _testMode = false;
...
function _loadDemoUser() {
  try {
    ...
    if (saved) {
      ...
      // If it's a test-mode user (uid starts with 'test-'), enable test mode
      if (u && u.uid && u.uid.startsWith('test-')) _testMode = true;
      _currentUser = u;
    }
  } catch(e) {}
}
...
function authGetCurrentUser() {
  if (_testMode) return _currentUser;
...
function authOnChange(callback) {
  if (!_testMode && FIREBASE_READY && _firebaseAuth) {
...
/** Set a test/demo user directly, bypassing Firebase auth */
function authSetDemoUser(user) {
  _testMode = true;
  _saveDemoUser(user);
}
